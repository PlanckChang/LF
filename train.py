...
from utils.loss_func import Weighted_Rec_Loss, Edge_Aware_Smoothness_Loss, Conf_Prior_Loss
...
parser.add_argument("--net_name", type=str, default='jingjin_LF')
...
def main():
    ...
    # model dir
    model_dir = '{}_Coarse_ckp_{}_an{}_sm{}_cf{}_ps{}_bs{}_lr{}'.format(
        opt.net_name, opt.dataset, opt.angular_num,opt.weight_smooth, opt.weight_conf, opt.patch_size, opt.batch_size , opt.lr)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    # model_dir_tem = 'Coarse_ckp_{}_an{}_sm{}_cf{}_ps{}_bs{}_lr{}'.format(
    ...
    for epoch in range(opt.resume_epoch + 1, opt.max_epoch):
        ...
        for k in range(50):
            for t, batch in enumerate(train_loader, 1):
                ...
                for s in range(len(out_disp_coarse)):
                    ...
                    loss += loss_coarse_rec + opt.weight_smooth*loss_coarse_smooth
        ...
        print(f'epoch {epoch} loss {loss_count/ len(train_loader)}')
        # checkpoint
